...
-- ============================================================
-- 26. GRANTS — PUBLIC role (SPCS JUDGE user and read-only access)
-- ============================================================
...
-- Write access on all tables (approvals, ground truth, master data, etc.)
GRANT INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA RECONCILIATION.PUBLIC TO ROLE PUBLIC;

-- Stage access (upload + Cortex TO_FILE reads)
GRANT READ, WRITE ON STAGE RECONCILIATION.PUBLIC.DOCUMENTS_STAGE_SSE  TO ROLE PUBLIC;
GRANT READ        ON STAGE RECONCILIATION.PUBLIC.DOCUMENTS_STAGE       TO ROLE PUBLIC;
GRANT READ        ON STAGE RECONCILIATION.PUBLIC.DOCUMENTS_STAGE_UNENC TO ROLE PUBLIC;

-- Stored procedure access
GRANT USAGE ON PROCEDURE RECONCILIATION.PUBLIC.EXTRACT_DOCUMENT_MULTIMODAL(VARCHAR, VARCHAR) TO ROLE PUBLIC;
GRANT USAGE ON PROCEDURE RECONCILIATION.PUBLIC.EXTRACT_ALL_MULTIMODAL()                      TO ROLE PUBLIC;
GRANT USAGE ON PROCEDURE RECONCILIATION.PUBLIC.SYNC_CURATED_MASTER()                         TO ROLE PUBLIC;
GRANT USAGE ON PROCEDURE RECONCILIATION.PUBLIC.APPLY_PROJECT_MERGES()                        TO ROLE PUBLIC;
